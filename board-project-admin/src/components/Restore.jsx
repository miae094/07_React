...
export default function Restore() {
  ...
  const restoreMember = async (member) => {
    // 복구 요청
    // 복구되었음 하고 응답 옴
    // -> getWithdrawnMemberList() 함수 재 호출해야하기 때문에 부모에 함수 만듦
    if (
      window.confirm(member.memberNickname + "님을 탈퇴 복구 시키시겠습니까?")
    ) {
      try {
        ...
        if (resp.status === 200) {
          ...
          // 복구된 데이터 확인(업데이트 된 내역을 확인하기 위해서)
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const restoreBoard = async (board) => {
    if (window.confirm(board.boardNo + " 번 게시글을 복구 시키시겠습니까?")) {
      try {
  ...
  // Restore 컴포넌트가 화면에 처음 렌더링(마운트) 될 때
  useEffect(() => {
    getWithdrawnMemberList();
    getDeleteBoardList();
  }, []);

  /* 상태값 null 	-> 서버에게서 응답 받기 전
   상태값 [] 		-> 서버에게 응답 받음(조회된 데이터 없음)
   상태값 [요소] 	-> 서버에게 응답 받음(조회된 데이터 있음)
 */

  // withdrawnMembers, deleteBoards 상태가 변경될 때 실행(isLoading 값 변경)
  useEffect(() => {
    if (deleteBoards != null && deleteBoards != null) {
      setIsLoading(false);
    }
