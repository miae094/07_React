...
//              불필요한 props drilling(상태 내리꽂기) 을 방지함.
//              사용하기 위해서는 useContext hook 필요함
...
// 부모 컴포넌트
const Exam6 = () => {
  const [user, setUser] = useState("블라썸");

  // Context.Provider : 하위 컴포넌트에게 데이터를 전달할때(제공할때) 사용
  return (
...
const Child2 = () => {
  ...
  return (
    <>
      <h1>
        I'm Child2 부모재산 쓰고시퍼 : {ContextValue.user} / {ContextValue.test}
      </h1>
      <Child3 />
    </>
  );
...
const Child3 = () => {
  ...
  const ContextValue = useContext(UserContext);
  // Context 로 등록된 객체 중 이름이 UserContext를 꺼내어 데이터 사용(소비)하겠다
  return (
    <>
      <h1>
